...
class _CustomerDashboardState extends State<CustomerDashboard> {
  ...
  void _rateProvider(String service, String mobile) async {
    ...
    if (rating != null) {
      await PreferencesService.saveRating(service, mobile, rating);
      setState(() {});
    }
  }

  Future<double> _getProviderRating(String service, String mobile) async {
    return await PreferencesService.getRating(service, mobile);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<Map<String, List<ProviderModel>>>(
                      ...
                      child: ExpansionTile(
                        ...
                        children: entry.value.map((provider) {
                          return FutureBuilder<double>(
                            future: _getProviderRating(provider.service, provider.phone),
                            builder: (context, ratingSnapshot) {
                              ...
                              return Container(
                                ...
                                child: ListTile(
                                  ...
                                  isThreeLine: true,
                                  trailing: SizedBox(
                                    width: 96, // enough width for two icons
                                    child: Row(
                                      mainAxisAlignment: MainAxisAlignment.end,
                                      children: [
                                        IconButton(
                                          icon: const Icon(Icons.rate_review, color: Colors.orange),
                                          onPressed: () => _rateProvider(provider.service, provider.phone),
                                          tooltip: 'Rate',
                                        ),
                                        IconButton(
